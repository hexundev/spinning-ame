...
s32 lut_cos(u16 theta);
s32 lut_sin(u16 theta);
...
INLINE void obj_aff_identity(OBJ_AFFINE *objAff)
{
	objAff->pa = objAff->pd = 1<<8;
	objAff->pb = objAff->pc = 0;
}

INLINE void obj_aff_rotate(OBJ_AFFINE *oaff, u16 alpha, int sx, int sy)
{
	int ss = lut_sin(alpha);
	int cc = lut_cos(alpha);
	oaff->pa = cc*sx>>12;
	oaff->pb = -ss*sx>>12;
	oaff->pc = ss*sy>>12;
	oaff->pd = cc*sy>>12;
